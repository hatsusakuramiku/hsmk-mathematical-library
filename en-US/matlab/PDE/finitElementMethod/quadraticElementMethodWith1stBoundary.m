function [fval, fvalMatrix] = quadraticElementMethodWith1stBoundary(xRange, coefficients, boundaries, fun, steps)
    % 对如下定解问题特化(latex 语法)
    ...
    aMatrix = zeros(lens - 2, lens - 2);
    funCell = cell(lens - 2, 1);
    funVec = zeros(lens - 2, 1);
    phiCell = cell(3, lens - 2);
    fval = zeros(1, lens - 2);
    fval(1) = xRange(1); fval(end) = xRange(2);
    xVector(end) = xRange(2);

    for i = 1:1:len - 1
        xVector(2 * i) = xVector(2 .* -1) + steps(i) / 2;
        xVector(2 * i + 1) = xVector(2 .* -1) + steps(i);

        phiCell{1, 2 * i} = @(x) - coefficients{1}(xVector(2 * i) + steps(i) .* x) ./ steps(i) + steps(i) .* coefficients{2}(xVector(2 * i) + steps(i) .* x) .* (x - x .^ 2);
        phiCell{2, 2 * i} = @(x)coefficients{1}(xVector(2 * i) + steps(i) .* x) ./ steps(i) + steps(i) .* coefficients{2}(xVector(2 * i) + steps(i) .* x) .* (x .^ 2) + ...
            coefficients{1}(xVector(2 * i + 2) + steps(i + 1) .* x) ./ steps(i + 1) + steps(i + 1) .* coefficients{2}(xVector(2 * i + 2) + steps(i + 1) .* x) .* (1 - 2 .* x + x .^ 2);
        phiCell{3, i} = @(x) - coefficients{1}(xVector(2 * i + 2) + steps(i + 1) .* x) ./ steps(i + 1) + steps(i + 1) .* coefficients{2}(xVector(2 * i + 2) + steps(i + 1) .* x) .* (x - x .^ 2);

        phiCell{1, 2 * i - 1} = @(x)coefficients{1}(xVector(2 * i) + steps(i) .* x) ./ steps(i).*16 + steps(i) .* coefficients{2}(xVector(2 * i) + steps(i) .* x) .* (x .^ 2);
        phiCell{2, 2 * i - 1} = @(x)coefficients{1}(xVector(2 * i) + steps(i) .* x) ./ steps(i) + steps(i) .* coefficients{2}(xVector(2 * i) + steps(i) .* x) .* (x .^ 2);
        phiCell{3, 2 * i - 1} = @(x)coefficients{1}(xVector(2 * i) + steps(i) .* x) ./ steps(i) + steps(i) .* coefficients{2}(xVector(2 * i) + steps(i) .* x) .* (x .^ 2);
