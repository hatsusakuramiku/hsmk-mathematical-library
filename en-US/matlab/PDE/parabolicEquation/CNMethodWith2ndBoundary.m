function [fval, fvalMatrix] = CNMethodWith2ndBoundary(xRange, initial, coefficient, fun, conditions, xNum, tNum, tStep)
    ...
    elseif gridRadio > 0.5
        warning('MATLAB:forwordDifferenceWith1stBoundary:InvalidInput', '网格比太大，会导致计算不稳定。');
    end
    ...
    boundaryMatrix = get2ndBoundaryPoint(xRange, initial, conditions{1}, conditions{2}, tStep, tNum, 1);
    fvalMatrix(:, 1) = boundaryMatrix(1:end - 1, 1);
    fvalMatrix(:, end) = boundaryMatrix(1:end - 1, 2);
    ...
    for i = 2:1:tNum + 1
        funVector_ = [funVector(1) + gridRadio * (boundaryMatrix(i, 1) + boundaryMatrix(i + 1, 1)), funVector(2:end - 1), ...
                          funVector(end) + gridRadio * (boundaryMatrix(i, 2) + boundaryMatrix(i - 1, 2))];
