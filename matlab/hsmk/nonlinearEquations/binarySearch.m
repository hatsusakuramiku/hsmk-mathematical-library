function [point, fval, fittingMatrix] = binarySearch(inputFunction, area, accuracy)
    %BINARYSEARCH 二分查找法
    %   [point, fval, fittingMatrix] = binarySearch(inputFunction, area, accuracy)
    %   Inputs
    ...
    if length(symvar(inputFunction)) > 1
        throw(MException('Matlab:binarySearch:InvalidInput', sprintf("输入的公式 %s 中的自变量格式不唯一，本函数仅支持单个自变量!", inputFunction)));
    end
