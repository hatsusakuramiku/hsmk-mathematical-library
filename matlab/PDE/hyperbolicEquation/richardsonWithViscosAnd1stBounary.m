function [fval, fvalMatrix] = richardsonWithViscosAnd1stBounary(xRange, initial, coefficient, fun, conditions, xNum, tNum, tStep)
    % 适用方程:
    % $$ \frac{u^{n + 1}_j - u^n_j}{\tau} = a\frac{u^n_{j + 1} - 2u^n_j + u^n_{j - 1}}{h^2} + b\frac{u^n_{j + 1} - u^n_{j - 1}}{2h} $$
    ...
    matrix = (gridRadio_1 + gridRadio_2) .* aMatrix + (1 - 2 * gridRadio_1) .* bMatrix + (gridRadio_1 - gridRadio_2) .* cMatrix;
    xVector = xRange(1) + xStep:xStep:xRange(2) - xStep;
    funvector = func(fun, xVector) .* tStep;
    funvector(1) = conditions(1) * (gridRadio_1 - gridRadio_2);
    funvector(end) = conditions(2) * (gridRadio_1 + gridRadio_2);
