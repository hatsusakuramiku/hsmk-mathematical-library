classdef LagrangeInterpolation < Coordinates
    ...
    methods
        ...
        function lagrangeFunSeris = LagrangeInterpolation(xCoordinate, yCoordinate)
            %LAGRANGEINTERPOLATION 拉格朗日插值构造函数
            %   lagrangeFunSeris = LAGRANGEINTERPOLATION(xCoordinate, yCoordinate)
            %   Inputs:
            %       xCoordinate: x坐标
            %       yCoordinate: y坐标
        ...
        function polynomia = getPolynomia(lagrangeFunSeris, option)
            % 获取拉格朗日插值多项式
            %   polynomia = getPolynomia(lagrangeFunSeris, option)
            %   Inputs:
            %       lagrangeFunSeris: LagrangeInterpolation对象
            narginchk(1, 2);
